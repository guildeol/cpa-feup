...
void sieve_parallel(uint64_t n, uint32_t threads) 
{
    PapiHelper ph;
    uint64_t k = 3;
    uint64_t unprocessed_blocks;
    ph.startCounting();

    do
    {
        unprocessed_blocks = threads;
        
        #pragma omp parallel num_threads(threads)
        {
            #pragma omp task shared(primes, unprocessed_blocks) firstprivate(k)
            {
                bool abort = false;
                #pragma omp critical
                {
                    if (unprocessed_blocks > 0)
                        unprocessed_blocks--;
                    else
                        abort = true;
                }
                ...
                if (!abort)
                {
                    ...
                    uint64_t low_value = BLOCK_LOW(unprocessed_blocks, threads, n - 1);
                    uint64_t high_value = 2 + BLOCK_HIGH(unprocessed_blocks, threads, n -1);
                    uint64_t block_size = BLOCK_SIZE(unprocessed_blocks, threads, n - 1);
